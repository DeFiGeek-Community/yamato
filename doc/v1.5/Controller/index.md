...
- **説明**: 重みの投票を変更することができるのは10日に1回だけです。現在の値は10日（864,000秒）です。

### `MULTIPLIER: constant(uint256) = 10 ** 18`
- **説明**: 多数の固定小数点数を扱うための乗数。現在の値は10の18乗です。
...
- **説明**: GaugeControllerの所有権を`addr`に転送するためのコミット関数。
- **パラメータ**:
  - `addr`: 所有権が転送されるアドレス。
...
- **説明**: 指定されたタイムスタンプでのゲージの相対重み（1.0を超えない）を取得します。
- **パラメータ**:
  - `addr`: ゲージのアドレス。
  - `time`: 指定したタイムスタンプ。
...
- **パラメータ**:
  ...
  - `_user_weight`: ゲージの重み (bps単位、0.01%の単位)。
