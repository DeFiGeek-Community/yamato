...
Curve DAO Tokenは、piecewise-linear mining supplyを持つERC20トークンです。このトークンのサプライは線形に増加しており、マイニングパラメータは定期的に更新される。初期供給は一定であり、その後の供給レートは時間の経過とともに減少していく。
...
- **説明**: 1年の秒数を表す定数。具体的には、1日の秒数（86400秒）に365を掛けた値。
...
- **説明**：_ownerが_spenderに許可したトークンの量を返します。
- **パラメータ**：
  - `_owner`：資金の所有者のアドレス。
  - `_spender`：資金を使うアドレス。
