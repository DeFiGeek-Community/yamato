...
(async ()=>{
    ...
    */
    ...
    await ( await EthPriceAggregatorInUSD.connect(getFoundation()).latestRoundData({gasLimit: 200000}) ).wait();
    await ( await JpyPriceAggregatorInUSD.connect(getFoundation()).latestRoundData({gasLimit: 200000}) ).wait();
    await ( await TellorCaller.connect(getFoundation()).getTellorCurrentValue(59, {gasLimit: 200000}) ).wait();
    ...
    */
    let dumpForDetectingBothUntrustedFlow1 = await feed.dumpForDetectingBothUntrustedFlow().catch(e=> console.trace(e.message) )
    let dumpForDetectingGasEater = await feed.dumpForDetectingGasEater().catch(e=> console.trace(e.message) )
    let dumpForChainLinkNow = await feed.dumpForChainLinkNow(0).catch(e=>{})
    let dumpForChainLinkBefore = await feed.dumpForChainLinkBefore(0).catch(e=>{})
    ...
    */
    console.log('calling')
    let tx = await feed.connect(getFoundation()).fetchPrice({gasLimit:3000000}).catch(e=> console.trace(e.message) )
    console.log('waiting', tx.hash.substr(0,8))
    let res = await tx.wait();

    let dumpForDetectingBothUntrustedFlow2 = await feed.dumpForDetectingBothUntrustedFlow().catch(e=> console.trace(e.message) )
    console.log(`dumpForDetectingBothUntrustedFlow2: ${dumpForDetectingBothUntrustedFlow2}`)
    ...
    let dumpForChainLinkNow2 = await feed.dumpForChainLinkNow(0).catch(e=> console.trace(e.message) )
    let dumpForChainLinkBefore2 = await feed.dumpForChainLinkBefore(0).catch(e=> console.trace(e.message) )
    let dumpForTellor2 = await feed.dumpForTellor(0).catch(e=> console.trace(e.message) )
