...
>(olderInstanceAddress:string, contractNameTo: string): Promise<T> {
    ...
    const instance: T = <T>(
      await upgrades.upgradeProxy(olderInstanceAddress, contractFactory, { kind: "uups" })
    );
    return instance;
}
...
export async function upgradeLinkedProxy<
    T extends BaseContract,
    S extends ContractFactory
>(olderInstanceAddress:string, contractNameTo: string, libralies: string[]): Promise<T>{
    let Libraries = {};
    ...
    const newerInstance: T = <T>await upgrades.upgradeProxy(olderInstanceAddress, contractFactory, {
        kind: "uups",
