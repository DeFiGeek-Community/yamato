...
let signedContracts = {};
export async function forge<T=Contract>(contractName:string, ABI:any, args:Array<any> = [], signer:Signer|undefined=undefined, linkings:Array<string>|undefined=undefined):Promise<T>{
...
export async function _summon<T=Contract>(contractName:string, ABI:any, args:Array<any> = [], signer:Signer|undefined=undefined, singleton:boolean=true, linkings:Array<string>):Promise<T>{
  ...
  if( singleton && !!signedContracts[contractName] ) {
    ...
    const _Contract = await forge(contractName, ABI, !args ? [] : args, !signer ? undefined : signer, linkings);

    let contract = new ethers.Contract(_Contract.address, ABI, getSharedProvider());
