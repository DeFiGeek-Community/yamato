...
describe("GaugeController", function () {
  ...
  describe("test gauges weights", function () {
    it("test addGauges", async () => {
      ...
      expect(await setup.gaugeController.gauges(0)).to.equal(setup.gaugesAddress[0]);
      expect(await setup.gaugeController.gauges(1)).to.equal(setup.gaugesAddress[1]);
    });
    ...
    it("test nGauges same gauge", async () => {
      ...
      await setup.gaugeController.addGauge(setup.gaugesAddress[0], 0, 0);
      await expect(setup.gaugeController.addGauge(setup.gaugesAddress[0], 0, 0)).to.revertedWith(
    ...
    it("test nGaugeTypes", async () => {
      ...
      expect(await setup.gaugeController.nGaugeTypes()).to.equal(BigNumber.from("2")); //unset, LiquidityGauge, Liquidity
    });
    ...
    it("test gaugeTypes", async () => {
      ...
      expect(await setup.gaugeController.gaugeTypes(setup.gaugesAddress[0])).to.equal("1");
      expect(await setup.gaugeController.gaugeTypes(setup.gaugesAddress[1])).to.equal("0");
    });

    it("test gauge weight", async () => {
      await setup.gaugeController.addGauge(setup.gaugesAddress[0], 0, setup.ten_to_the_19);

      expect(await setup.gaugeController.getGaugeWeight(setup.gaugesAddress[0])).to.equal(setup.ten_to_the_19);
    });

    it("test gauge weight as zero", async () => {
      await setup.gaugeController.addGauge(setup.gaugesAddress[0], 0, 0);

      expect(await setup.gaugeController.getGaugeWeight(setup.gaugesAddress[0])).to.equal("0");
    });
    ...
    it("test getGaugeWeight", async () => {
      ...
      await setup.gaugeController.changeGaugeWeight(setup.gaugesAddress[0], setup.ten_to_the_21);

      expect(await setup.gaugeController.getGaugeWeight(setup.gaugesAddress[0])).to.equal(setup.ten_to_the_21);
    });

    it("test type weight", async () => {
      await setup.gaugeController.addType("Liquidity", setup.TYPE_WEIGHTS[0]);
      expect(await setup.gaugeController.getTypeWeight(1)).to.equal(setup.TYPE_WEIGHTS[0]);
    ...
    it("test changeTypeWeight", async () => {
      ...
      expect(await setup.gaugeController.getTypeWeight(2)).to.equal(setup.TYPE_WEIGHTS[1]);
    });
    ...
    it("test relative weight write", async () => {
      ...
      await setup.gaugeController.addGauge(setup.gaugesAddress[0], 1, setup.GAUGE_WEIGHTS[0]);
      await setup.gaugeController.addGauge(setup.gaugesAddress[1], 1, setup.GAUGE_WEIGHTS[1]);
      await setup.gaugeController.addGauge(setup.gaugesAddress[2], 2, setup.GAUGE_WEIGHTS[2]);
      ...
      for (let i = 0; i < 3; i++) {
        await setup.gaugeController.gaugeRelativeWeightWrite(setup.gaugesAddress[i], t);
        let relativeWeight = await setup.gaugeController.gaugeRelativeWeight(setup.gaugesAddress[i], t);
