...
describe("ERC20CRV", function () {
  ...
  describe("ERC20CRV MintableInTimeframe", function () {
    ...
    async function theoreticalSupply(token: Contract): Promise<BigNumber> {
      ...
      S = S.add(
        YEAR_1_SUPPLY.div(YEAR)
          .mul(q.pow(epoch))
          .mul((await ethers.provider.getBlock("latest")).timestamp - (await token.startEpochTime()))
      );
    ...
    it("test_mintable_in_timeframe", async function () {
      ...
      expect((availableSupply.sub(INITIAL_SUPPLY.mul(Constants.ten_to_the_18)).gte(mintable))).to.equal(true);
      if (t1 == t0) {
        expect(mintable).to.equal(BigNumber.from(0));
      } else {
        const tolerance = BigNumber.from("10000000"); // Adjust as needed for precision
        expect(availableSupply.sub(INITIAL_SUPPLY.mul(Constants.ten_to_the_18)).div(mintable).sub(1)).to.be.lt(tolerance);
      }
      ...
      expect(approx(await theoreticalSupply(token), availableSupply, Constants.ten_to_the_16)).to.equal(true);
    });
    ...
    it("test_random_range_year_one", async function () {
      ...
      expect(await token.mintableInTimeframe(sortedTimes[0], sortedTimes[1])).to.equal(rate.mul(sortedTimes[1].sub(sortedTimes[0])));
    });
    ...
    it("test_random_range_multiple_epochs", async function () {
      ...
      const rate = YEAR_1_SUPPLY.div(YEAR).div(BigNumber.from(2).pow(exponent.div(100)));
      ...
      if (startEpoch.eq(endEpoch)) {
        ...
        expect(approx(mintable, expectedMintable, Constants.ten_to_the_16)).to.be.true;
      } else {
