...
describe("LiquidityGauge", function () {
  ...
  beforeEach(async () => {
    ...
    ({ gaugeController, threeGauges, gauges, votingEscrow, token, mockLpToken } =
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (is_withdraw) {
          ...
          let amount = BigNumber.from(Math.floor(Math.random() * 10000).toString())
            ...
            .div(BigNumber.from("10000"));
          await mockLpToken.connect(accounts[2]).approve(threeGauges[0], amount);
          await gauges[0].connect(accounts[2]).deposit(amount, accounts[2].address, false);
          await update_integral();
        ...
        // Aliceの処理
        if (is_alice) {
          let is_withdraw_alice = (await gauges[0].balanceOf(accounts[1].address)) > 0 && Math.random() > 0.5;
          if (is_withdraw_alice) {
            ...
            let amount_alice = BigNumber.from(Math.floor(Math.random() * 10000).toString())
              ...
              .div(BigNumber.from("10000"));
            await mockLpToken.connect(accounts[1]).approve(threeGauges[0], amount_alice);
            await gauges[0].connect(accounts[1]).deposit(amount_alice, accounts[1].address, false);
            await update_integral();
        ...
        // 保有量の確認
        expect(await gauges[0].balanceOf(accounts[1].address)).to.equal(alice_staked);
        expect(await gauges[0].balanceOf(accounts[2].address)).to.equal(bob_staked);
        expect(await gauges[0].totalSupply()).to.equal(alice_staked.add(bob_staked));
  ...
  describe("Mining with Vote Locking", function () {
    it("should distribute rewards according to vote lock status", async () => {
      ...
      await token.connect(accounts[2]).approve(votingEscrow.address, MAX_UINT256);
      const creatorBalance = await mockLpToken.balanceOf(accounts[0].address);
      await mockLpToken.transfer(
        accounts[1].address,
        (creatorBalance).div(BigNumber.from("2"))
      ...
      await votingEscrow.connect(accounts[1]).createLock(ten_to_the_20, t.add(WEEK.mul(BigNumber.from("2"))));
    
      // AliceとBobが一部の流動性をデポジットする
      await gauges[0].connect(accounts[1]).deposit(ten_to_the_21, accounts[1].address, false);
      await gauges[0].connect(accounts[2]).deposit(ten_to_the_21, accounts[2].address, false);
      let now = BigNumber.from((await ethers.provider.getBlock("latest")).timestamp);
    
      // 現在、Aliceは投票ロックを持っているが、Bobは持っていないことを確認する
      expect(await votingEscrow["balanceOf(address,uint256)"](accounts[1].address, now)).to.not.equal(zero);
      expect(await votingEscrow["balanceOf(address,uint256)"](accounts[2].address, now)).to.equal(zero);
      ...
      now = BigNumber.from((await ethers.provider.getBlock("latest")).timestamp);
      expect(await votingEscrow["balanceOf(address,uint256)"](accounts[1].address, now)).to.equal(zero);
      expect(await votingEscrow["balanceOf(address,uint256)"](accounts[2].address, now)).to.equal(zero);
      
      // AliceはTokenを投票ロックしたので、2.5倍のTokenを獲得
      let rewards_alice = await gauges[0].integrateFraction(accounts[1].address);
