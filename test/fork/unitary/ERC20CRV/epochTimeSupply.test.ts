...
describe("ERC20CRV", function () {
  ...
  describe("ERC20CRV EpochTimeSupply", function () {
    ...
    it("test_update_mining_parameters_same_epoch", async function () {
      ...
      const now = BigNumber.from((await ethers.provider.getBlock("latest")).timestamp);
      const newEpoch = creationTime.add(YEAR).sub(now);
      await ethers.provider.send("evm_increaseTime", [newEpoch.sub(BigNumber.from("3")).toNumber()]);
      await expect(token.updateMiningParameters()).to.be.revertedWith("dev: too soon!");
    });

    it("test_mintable_in_timeframe_end_before_start", async function () {
      const creationTime = await token.startEpochTime();
      await expect(token.mintableInTimeframe(creationTime.add(1), creationTime)).to.be.revertedWith("dev: start > end");
    });
    ...
    it("test_mintable_in_timeframe_multiple_epochs", async function () {
      ...
      await token.mintableInTimeframe(creationTime, creationTime.add(YEAR).mul(BigNumber.from("19").div(BigNumber.from("10"))));

      // Three epochs should raise
      await expect(token.mintableInTimeframe(creationTime, creationTime.add(YEAR).mul(BigNumber.from("21").div(BigNumber.from("10"))))).to.be.revertedWith("dev: too far in future");
    });
