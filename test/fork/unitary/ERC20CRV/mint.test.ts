...
describe("ERC20CRV", function () {
  ...
  describe("ERC20CRV Mint", function () {
    it('test_available_supply', async function () {
      ...
      const currentBlock = BigNumber.from((await ethers.provider.getBlock("latest")).timestamp);
      // const currentBlock = await ethers.provider.getBlock('latest');
      const expected = initialSupply.add(currentBlock.sub(creationTime).mul(rate));
    ...
    it('test_zero_address', async function () {
      await token.setMinter(accounts[0].address);
      await expect(token.mint(ZERO_ADDRESS, 0)).to.be.revertedWith("dev: zero address");
    });
  });
});
