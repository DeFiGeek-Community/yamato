...
describe("GaugeController", function () {
  ...
  describe("test_vote", function () {
    it("test_vote", async () => {
      ...
      expect(
        await setup.gaugeController.voteUserPower(setup.accountsAddress[0])
      ).to.equal(BigNumber.from("10000")); //Total vote power used by user
    });
    ...
    it("test_vote_change", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[1], 1234)
    ...
    it("test_vote_no_balance", async () => {
      ...
      await expect(
        setup.gaugeController
          .connect(setup.accounts[1])
          .voteForGaugeWeights(setup.gaugesAddress[0], 10000)
    ...
    it("test_vote_expired", async () => {
      ...
      await ethers.provider.send("evm_increaseTime", [
        setup.YEAR.mul("2").toNumber(),
      ]);

      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[0], 10000)
      ).to.revertedWith("Your token lock expires too soon");
    });
    ...
    it("test_invalid_gauge_id", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[2], 10000)
    ...
    it("test_over_user_weight", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[0], 10001)
    ...
    it("test_over_weight_multiple", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[1], 4000)
    ...
    it("test_over_weight_adjust_existing", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[0], 8000)
      ).to.revertedWith("Used too much power");
    });
    ...
    it("test_exceeding_voting_power", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[1], 5000)
      ).to.revertedWith("Used too much power");
    });
    ...
    it("test_emit_vote_event", async () => {
      ...
      await expect(
        setup.gaugeController.voteForGaugeWeights(setup.gaugesAddress[0], 5000)
      )
