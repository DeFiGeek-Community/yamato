...
describe("Minter", function () {
  ...
  describe("Minter Behavior", function () {
    ...
    it("test mint", async () => {
      await setup.gaugesContracts[0]
        .connect(setup.accounts[1])
        .deposit(setup.ten_to_the_17, setup.accountsAddress[1]);

      await ethers.provider.send("evm_increaseTime", [setup.MONTH.toNumber()]);

      await setup.minter
        .connect(setup.accounts[1])
        .mint(setup.gaugesAddress[0]); //gauge_address, msg.sender, mint
      let expected = await setup.gaugesContracts[0].integrateFraction(
        setup.accountsAddress[1]
      );

      expect(expected.gt(BigNumber.from("0"))).to.be.equal(true);
      expect(await setup.token.balanceOf(setup.accountsAddress[1])).to.equal(
        expected
      );
      expect(
    ...
    it("test mint immediate", async () => {
      await setup.gaugesContracts[0]
        .connect(setup.accounts[1])
        .deposit(setup.ten_to_the_18, setup.accountsAddress[1]);
    ...
    it("test mint multiple gauges", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        expect(minted).to.equal(
          await gauge.integrateFraction(setup.accountsAddress[1])
        );
    ...
    it("test mint after withdraw", async () => {
      ...
      await setup.gaugesContracts[0]
        .connect(setup.accounts[1])
        .withdraw(setup.ten_to_the_18);
      await setup.minter
        .connect(setup.accounts[1])
        .mint(setup.gaugesAddress[0]);

      expect(
        (await setup.token.balanceOf(setup.accountsAddress[1])).gt(
          BigNumber.from("0")
        )
      ).to.equal(true);
    });
    ...
    it("test mint not a gauge", async () => {
      await expect(setup.minter.mint(setup.accountsAddress[1])).to.revertedWith(
    ...
    it("test mintFor function", async () => {
      ...
      await setup.minter
        ...
        .mintFor(setup.gaugesAddress[0], setup.accountsAddress[1]);
