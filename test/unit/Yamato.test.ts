...
describe("Smock for Yamato", function () {
  it(`succeeds to make a mock`, async function () {
    ...
    const spec2 = await ethers.getContractFactory("Yamato", {
      libraries: { PledgeLib: pledgeMock.address },
    });
...
describe("contract Yamato", function () {
  ...
  beforeEach(async () => {
    ...
    const spec6 = await ethers.getContractFactory("PriorityRegistry", {
      libraries: { PledgeLib },
    });
    ...
    mockPool.smocked.sweepReserve.will.return.with(
    ...
    mockPriorityRegistry.smocked.popRedeemable.will.return.with(
      encode(
    ...
    mockPriorityRegistry.smocked.popSweepable.will.return.with(
      encode(
        ["uint256", "uint256", "bool", "address", "uint256"],
        [
          BigNumber.from(0),
  ...
  describe("deposit()", function () {
    ...
    it(`should run upsert`, async function () {
      const toCollateralize = 1;
      await yamato.deposit({ value: toERC20(toCollateralize + "") });
      betterexpect(mockPriorityRegistry.smocked.upsert.calls.length).toBeGteBN(
        1
      );
    });
  });
  ...
  describe("borrow()", function () {
    ...
    it(`should run CjpyOS.mintCJPY() of Pool.sol`, async function () {
      ...
      betterexpect(
        mockCjpyOS.smocked["mintCJPY(address,uint256)"].calls.length
      ).toBe(2);
    });
    it(`should run depositRedemptionReserve when RR is inferior to SR`, async function () {
      ...
      await yamato.borrow(toERC20(toBorrow + ""));
      betterexpect(
        mockPool.smocked["depositRedemptionReserve(uint256)"].calls.length
      ).toBe(1);
      betterexpect(
        mockPool.smocked["depositSweepReserve(uint256)"].calls.length
      ).toBe(0);
    });
  ...
  describe("repay()", function () {
    ...
    it(`should reduce debt`, async function () {
      ...
      betterexpect(pledgeBefore.debt.toString()).toBe(
        "236363636363636350000000"
      );
    ...
    it(`should improve TCR`, async function () {
      ...
      betterexpect(TCRafter.toString()).toBe(
    ...
    it(`should run burnCJPY`, async function () {
      ...
      await yamato.repay(toERC20(toBorrow + ""));
      betterexpect(
        mockCjpyOS.smocked["burnCJPY(address,uint256)"].calls.length
      ).toBe(1);
    });
  ...
  describe("withdraw()", function () {
    ...
    it(`should reduce coll`, async function () {
      ...
      betterexpect(pledgeAfter.coll.toString()).toBe(
        toERC20((toCollateralize * 9) / 10 + "").toString()
      );
  ...
  describe("redeem()", function () {
    ...
    it(`should expense coll of lowest ICR pledges even if price change make diff between LICR and real ICR`, async function () {
      ...
      await yamato
        .connect(accounts[0])
        .redeem(toERC20(toBorrow * 3 + ""), false);
    ...
    it(`should NOT run useRedemptionReserve\(\) of Pool.sol when isCoreRedemption=false`, async function () {
      await yamato.connect(accounts[0]).redeem(toERC20(toBorrow + ""), false);
      betterexpect(
        mockPool.smocked["useRedemptionReserve(uint256)"].calls.length
      ).toBe(0);
    });
    ...
    it(`should NOT run accumulateDividendReserve\(\) of Pool.sol when isCoreRedemption=false`, async function () {
      await yamato.connect(accounts[0]).redeem(toERC20(toBorrow + ""), false);
      betterexpect(
        mockPool.smocked["accumulateDividendReserve(uint256)"].calls.length
      ).toBe(0);
    });
    ...
    it.todo(
      `should not reduce CJPY when there're no ICR<MCR && coll>0 pledges`
    );
  });
  ...
  describe("sweep()", function () {
    ...
    it(`should improve TCR after sweeping`, async function () {
      ...
      */
      ...
      betterexpect((await priorityRegistry.nextSweepable()).isCreated).toBe(
        true
      );
