import { setNetwork, getDeploymentAddressPathWithTag, getFoundation, setProvider } from "../../src/deployUtil";
...
(async () => {
  ...
  for (const contractName of contracts) {
    const proxyAddress = readFileSync(getDeploymentAddressPathWithTag(contractName, "ERC1967Proxy")).toString();
    contractInstances[contractName] = new ethers.Contract(proxyAddress, genABI(contractName), getFoundation());
  }
  ...
  async function checkImplementation(contractInstance, versionTag) {
    ...
    const expectedImpl = readFileSync(getDeploymentAddressPathWithTag(versionTag, "UUPSImpl")).toString();
  ...
  for (const [contractName, versionTag] of Object.entries(versions)) {
    await checkImplementation(contractInstances[contractName], `${contractName}${versionTag}`);
  }
})();