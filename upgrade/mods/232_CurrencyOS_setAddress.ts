...
async function main() {
  ...
  if (process.env.NETWORK === "localhost") {
    ...
    await executeTransaction(CONTRACT_ADDRESS, implArtifact.abi, "setYMT", [
      ymtAddr,
    ]);
    await executeTransaction(CONTRACT_ADDRESS, implArtifact.abi, "setVeYMT", [
      veYmtAddr,
    ]);
    await executeTransaction(CONTRACT_ADDRESS, implArtifact.abi, "setYmtMinter", [
      ymtMinterAddr,
    ]);
